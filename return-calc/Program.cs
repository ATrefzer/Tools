...
using ReturnCalc.Calculators;
using ReturnCalc.Contracts;
using ReturnCalc.Converters;
using ReturnCalc.Models;
using ReturnCalc.Services;
using ReturnCalc.Strategies;

namespace ReturnCalc;

internal static class Program
{
    /// <summary>
    ///     ishares core msci world (acc)
    ///     return-calc EUNL.DE --buy 2010-01-03 --prices >EUNL.DE.json
    ///     return-calc GERD.DE --buy 2010-01-03 --prices >GERD.DE.json
    /// </summary>
    /// <returns></returns>
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            await Console.Error.WriteLineAsync("Usage: return-calc <ticker> [--buy yyyy-MM-dd] [--sell yyyy-MM-dd]");
        ...
        try
        {
            if (IsSimulationMode(args))
            {
                return await RunSimulationModeAsync(args);
            }
            ...
            if (searchIndex >= 0 && searchIndex + 1 < args.Length)
            {
                ...
                return await SimulateAsync(ticker);
            }

            var pricesMode = args.Contains("--prices");
            request = CreateRequestDto(args);


            if (pricesMode)
            {
                return await FetchPricesAsync(request);
            }

            var result = await CalculateReturnAsync(request);
            Console.WriteLine(JsonSerializer.Serialize(result, BacktestJsonContext.Default.ResultDto));
        }
        catch (Exception e)
        {
    ...
    private static bool IsSimulationMode(string[] args)
    {
        return args.Contains("--simulate");
    }

    private static async Task<int> FetchPricesAsync(TickerTimeRange request)
    {
    ...
    private static async Task<int> SimulateAsync(string query)
    {
    ...
    private static TickerTimeRange CreateRequestDto(string[] args)
    {
        string? ticker = null;
        string? buyStr = null;
        string? sellStr = null;
    ...
    private static async Task<ResultDto> CalculateReturnAsync(
        TickerTimeRange request)
    {
        var yahoo = new YahooFinanceService();
    ...
    private static async Task<int> RunSimulationModeAsync(string[] args)
    {
        // --simulate --list
        ...
        var result = SimulationStrategyRunner.Run(strategy, prices);
